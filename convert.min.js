...
- landing: 启用落地节点功能（如落地分组，默认 true）
...
function buildFeatureFlags(args) {
    ...
    for (const [sourceKey, targetKey] of Object.entries(spec)) {
        if (typeof args[sourceKey] !== "undefined") {
            flags[targetKey] = parseBool(args[sourceKey]);
        }
    }
...
function buildBaseLists({ landing, countryGroupNames }) {
...
const baseRules = [
    ...
    `DOMAIN-SUFFIX,zz1cc.cc.cd,${PROXY_GROUPS.SELECT}`,
    `DOMAIN-SUFFIX,xian995.dpdns.org,${PROXY_GROUPS.REVERSE_API}`,
    ...
    `DOMAIN,oauth2.googleapis.com,${PROXY_GROUPS.SELECT}`,
    `DOMAIN,accounts.google.com,${PROXY_GROUPS.SELECT}`,
...
const LANDING_REGEX = /落地/i;
const LANDING_PATTERN = "(?i)落地";
    ...
    for (const country of countries) {
        ...
        if (!regexFilter) {
            ...
            groupConfig = {
                ...
                "include-all": true,
                filter: meta.pattern,
                ...(landing ? { "exclude-filter": landingExcludeFilter } : {}),
                type: groupType,
            };
        }
...
function main(config) {
    const resultConfig = {
        proxies: (config.proxies || []).map((proxy, index) => {
            const port = 8000 + index;
            const renamedProxy = {
                ...proxy,
                name: `${port}-${proxy.name || ""}`,
            };

            return landing && LANDING_REGEX.test(proxy.name || "")
                ? { ...renamedProxy, "dialer-proxy": "前置代理" }
                : renamedProxy;
        }),
    };
    ...
    const { defaultProxies, defaultProxiesDirect, defaultSelector, defaultFallback } =
        buildBaseLists({ landing, countryGroupNames });
    ...
    const listeners = resultConfig.proxies.map((proxy, index) => ({
        address: "0.0.0.0",
        name: `mixed-${8000 + index}`,
        port: 8000 + index,
        proxy: proxy.name,
        type: "mixed",
    }));
